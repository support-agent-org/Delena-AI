...
import type { ReplTool } from "@/tool/repl"
  ...
  return (
    <Show when={!shouldHide()}>
      <Switch>
        ...
        <Match when={props.part.tool === "repl"}>
          <Repl {...toolprops} />
        </Match>
        <Match when={true}>
...
function Repl(props: ToolProps<typeof ReplTool>) {
  const { theme } = useTheme()
  const isRunning = createMemo(() => props.part.state.status === "running")
  const code = createMemo(() => (props.metadata as any).code ?? "")
  const output = createMemo(() => stripAnsi(((props.metadata as any).output ?? "").trim()))
  const success = createMemo(() => (props.metadata as any).success !== false)
  const [expanded, setExpanded] = createSignal(false)
  const lines = createMemo(() => output().split("\n"))
  const overflow = createMemo(() => lines().length > 10)
  const limited = createMemo(() => {
  ...
  const title = createMemo(() => {
    if (isRunning()) return "# REPL"
    return success() ? "# REPL" : "# REPL (failed)"
  })

  return (
    <Switch>
      <Match when={(props.metadata as any).code !== undefined}>
        <BlockTool
          title={title()}
          part={props.part}
          spinner={isRunning()}
        ...
        >
          <box gap={1}>
            ...
            <Show when={output()}>
              <text fg={success() ? theme.text : theme.error}>{limited()}</text>
            </Show>
            <Show when={overflow()}>
              <text fg={theme.textMuted}>{expanded() ? "Click to collapse" : "Click to expand"}</text>
            </Show>
      ...
      <Match when={true}>
        <InlineTool icon=">" pending="Preparing REPL..." complete={props.input.code} part={props.part}>
          REPL
        </InlineTool>
      </Match>
    </Switch>
